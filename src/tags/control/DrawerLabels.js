...
const HtxLabels = observer(({ item, store }) => {
  const style = {
    marginTop: "1em",
    marginBottom: "1em",
    display: "flex",
  ...
  const onCancel = () => {
    store.lastBufferedRegion && store.lastBufferedRegion.cancel();
    store.setLastBufferedRegion(undefined);
  };

  const onSubmit = () => {
    store.lastBufferedRegion && store.lastBufferedRegion.ok();
    store.setLastBufferedRegion(undefined);
  };
  ...
  return (
    <Drawer
      ...
      visible={!!store.lastBufferedRegion}
      onClose={onCancel}
      style={style}
    >
...
Registry.addTag("labels", LabelsModel, inject("store")(HtxLabels));
